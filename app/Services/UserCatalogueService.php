...
class UserCatalogueService implements UserCatalogueServiceInterface
{
    ...
    public function paginate($request)
    {
        ...
        ->pagination($this->paginateSelect(), $condition, [], ['path' => 'user/catalogue/index'], $perPage, ['users']);
    ...
    public function destroy($id){
        DB::beginTransaction();
        try{
            $user = $this->userCatalogueRepository->delete($id); //Có thể sử dụng hàm delete() để xóa mềm, forceDelete() để xóa vĩnh viễn
            DB::commit();
            return true;
        }catch(\Exception $e){
            DB::rollback();
